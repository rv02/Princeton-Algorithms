...
class BSTNode(object):
    ...
    def next_larger(self):
        ...
        """
        if self.right is not None: 			#Case 1: Node has a right subtree, Go deep to leftmost node in right subtree or find min in right subtree
            return self.right.find_min()
        current = self
        while current.parent is not None and current is current.parent.right:		#Case 2:No right subtree, Go to the nearest ancestor for which given node would be in left subtree
